...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        next_obs_pred = self.dynamics_models[i](obs_acs_normalized)
        loss = self.loss_fn(obs_delta_normalized, next_obs_pred)
    ...
    def get_dynamics_predictions(
        ...
        obs_acs = torch.cat([obs, acs], dim=1)
        obs_acs_normalized = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-6)
        pred_next_obs = self.dynamics_models[i](obs_acs_normalized)
        pred_next_obs = pred_next_obs * (self.obs_delta_std + 1e-6) + self.obs_delta_mean
        pred_next_obs += obs
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        action_sequences = np.transpose(action_sequences, (1, 0, 2))
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            elite_mean, elite_std = np.mean(action_sequences, axis=0), np.std(action_sequences, axis=0)
            for i in range(self.cem_num_iters):
                ...
                rewards = self.evaluate_action_sequences(obs, action_sequences)
                assert rewards.shape == (self.mpc_num_action_sequences,)
                elites_indexes = np.argpartition(rewards, -self.cem_num_elites)[-self.cem_num_elites:]
                action_elites = action_sequences[elites_indexes]
                elite_mean = self.cem_alpha * np.mean(action_elites, axis=0) + (1 - self.cem_alpha) * elite_mean
                elite_std = self.cem_alpha * np.std(action_elites, axis=0) + (1 - self.cem_alpha) * elite_std
