...
class AWACAgent(DQNAgent):
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            next_qs = next_qa_values.gather(dim=1, index=next_actions)

            # TODO(student): Compute the TD target
            target_values = rewards.unsqueeze(dim=1) + self.discount * torch.logical_not(dones.unsqueeze(dim=1)) * next_qs
    ...
    def compute_advantage(
        ...
        q_values = qa_values.gather(dim=1, index=actions.unsqueeze(1))
        values = (qa_values * action_dist.probs).sum(dim=1, keepdim=True)
    ...
    def update_actor(
        ...
        loss = -1 * torch.mean(action_dist.log_prob(actions) * torch.exp(1.0/self.temperature * advantages))
